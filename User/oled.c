...
{	
	...
	{			  
		OLED_SCLK_Clr();
		if(dat&0x80)               //取出数据的最高位，判断1或0，然后对应SDIN(D1)输入到SSD1306
		   OLED_SDIN_Set();
		else 
		   OLED_SDIN_Clr();
		OLED_SCLK_Set();           //由时序图可以看出，数据/指令的写入是在SCLK的上升沿时进行的
		dat<<=1;                   //数据移位，从高位到低位写入
	}				 		  
	OLED_CS_Set();  	  
} 
...
//m^n函数
u32 oled_pow(unsigned int m,unsigned int n)
{
	u32 result=1;	 
	while(n--)result*=m;    
	return result;
...
//显示一个字符号串
void OLED_ShowString(unsigned int x,unsigned int y,unsigned char *chr)
{
	unsigned char j=0;
	while (chr[j]!='\0')
	{		
...
{ 	 
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50MHz
 	GPIO_Init(GPIOA, &GPIO_InitStructure);	  //初始化PA4,PA5,PA7
